#!/usr/bin/env python3
"""
E491xA_Firmware_Updater.py -- verify and upload E4916A/E4915A firmware images to
...
Two rules matter when talking to the option subsystem:

  * NEVER query "*OPT?" around an option write. It reports the options ACTIVE
    since the last boot (not what is stored in flash) and its reply is empty
    when none are active; reading an empty reply makes the instrument raise
    "GPIB ERROR: -420" (query unterminated). Use ":TEST:INSTR:INFO:DATA?",
    which always returns all six fields.

  * After a successful write the firmware re-reads the identity record and runs
    a subsystem re-initialisation chain, during which it has no reply ready.
    Enabling an option gives that re-init more work than disabling. So send
    "*WAI" and wait before reading anything back -- which is what this tool
    does (longer on enable than on disable).
"""
...
class Instrument:
    ...
    def clear_status(self) -> None:
        """Clear the status registers and error queue with ``*CLS``.

        ``*CLS`` is a command, not a query: it sends nothing back, so it can
        never leave the controller addressed-to-talk with an empty output queue
        (which is what raises ``-420 Query UNTERMINATED``). Prefer this over
        polling ``:SYSTem:ERRor?`` in a loop just to empty the queue.
        """
        try:
            self.write("*CLS")
    ...
    def wait_for_pending(self) -> None:
        """Send ``*WAI`` so the instrument finishes pending operations first.

        After a successful option write the firmware re-reads the identity
        record and runs a subsystem re-initialisation chain (``0xE1004E`` then
        ``0xE38E80``). During that window it has no reply ready, and reading it
        is what produces ``-420``. ``*WAI`` is a command with no response, so
        it cannot itself provoke a query error, and it makes the instrument
        complete the pending work before handling anything further.
        """
        try:
            self.write("*WAI")
        except Exception:  # noqa: BLE001 - best effort
    ...
    def read_option_record_raw(self, attempts: int = 3,
                               retry_delay_s: float = 1.0) -> Optional[str]:
        """Return the full identity/option record exactly as the instrument
        reports it, e.g. ``"E4916A","JP1KD00123","010","000","000","000"``.

        This is the authoritative "model, serial, options" string. Unlike
        ``*OPT?`` it always returns six fields, so a successful read can never
        be empty.

        Straight after an option write the instrument is busy programming the
        config flash sector and may not have a reply ready; reading it in that
        state is what the instrument reports as ``-420 Query UNTERMINATED``.
        So the read is retried a few times rather than issued once.
        ...
        Returns:
            The raw response, or ``None`` if every attempt failed.
        """
        for attempt in range(attempts):
            try:
                ...
                if response:
                    return response
            except Exception:  # noqa: BLE001 - busy / not ready yet
                pass
...
class Console:
    ...
    @staticmethod
    def signoff(delay_s: float = 1.0) -> None:
        """Print the parting message and hold it briefly before exiting."""
...
class FirmwareLoaderApp:
    ...
    def menu_scan(self) -> None:
        ...
        if matches:
            res, _idn = matches[0]
            self.resource = res
            print("\n(stopped at the first match; remembered for this session)")
        else:
    ...
    def menu_options(self) -> None:
        ...
        try:
            with Instrument(resource) as inst:
                ...
                # The full "model, serial, options" string, straight from the
                # instrument. This is the authoritative BEFORE state.
                groups, raw = inst.read_option_groups()
                if groups is None:
                    print("(couldn't read the option record; unknown groups "
                          "default to 000)")
                ...
                if not record.is_e4916a:
                    ...
                    if e4916a_only:
                        print(f"\nNote: {', '.join(e4916a_only)} are E4916A "
                              ...
                              "reject them -- compare the two records below.")

                # Show the proposed change and confirm the write.
                before = raw or OptionRecord.quote_fields([model, serial] + original)
                after = record.as_quoted()
                print(f"\n  current : {before}")
                print(f"  proposed: {after}")
                print(f"\nAbout to write (updates the config-flash option record):"
                      f"\n  {record.to_command()}")
                if Console.ask('\nType EXACTLY "WRITE" to proceed (anything else '
                               'cancels)') != "WRITE":
                    print("Cancelled.")
                    return

                # Start from a clean status. *CLS is a command (no response),
                # so unlike reading :SYST:ERR? it cannot itself provoke a -420.
                inst.clear_status()

                # Does this change turn any option ON? Enabling makes the
                # instrument's post-write re-initialisation do more work, so it
                # needs a longer settle before we read anything back.
                enabling = any(
                    record.is_enabled(opt) and
                    original[opt.group_index].upper() != opt.code
                    for opt in OptionRecord.OPTIONS)

                inst.write_option_record(record)

                # After a successful write the instrument re-reads its identity
                # record (0xE1004E) and runs a subsystem re-initialisation chain
                # (0xE38E80). Reading it during that window is what produced the
                # -420 errors. *WAI makes it finish first; then allow a settle,
                # longer when enabling because that re-init has more to do.
                inst.wait_for_pending()
                time.sleep(5.0 if enabling else 2.0)

                # Read the full record back as the authoritative AFTER state.
                # NOTE: *OPT? is deliberately never used here -- it reports the
                # options ACTIVE since the last boot (not what was just written
                # to flash), and its reply is empty when none are active, which
                # makes the instrument raise "GPIB ERROR: -420" (query
                # unterminated). The record query always returns six fields, so
                # it is always safe and shows model, serial and all options.
                readback = inst.read_option_record_raw()

                print(f"\nRecord BEFORE: {before}")
                if readback is None:
                    print("Record AFTER : (read back failed)")
                else:
                    print(f"Record AFTER : {readback}")

                # The BEFORE/AFTER records above already show whether the
                # change was applied, so the error queue is not read here: every
                # query is another addressed-to-talk transaction and therefore
                # another chance of a spurious -420. *CLS clears any error the
                # instrument logged during its post-write re-initialisation.
                inst.clear_status()
